#!/usr/bin/env python3
"""
递归移除文件夹下所有文本文件中以 System.Diagnostics.Debug.WriteLine 开头的行
"""
...
def remove_debug_lines(file_path, dry_run=False):
    """移除文件中的调试输出行"""
    try:
    ...
    # 匹配 System.Diagnostics.Debug.WriteLine 开头的行（考虑缩进）
    debug_pattern = re.compile(r'^\s*System\.Diagnostics\.Debug\.WriteLine.*$', re.MULTILINE)
...
def process_directory(directory, dry_run=False):
    ...
    for file_path in directory.rglob('*'):
        if file_path.is_file() and is_text_file(file_path):
            total_files += 1
            removed_count = remove_debug_lines(file_path, dry_run)
            if removed_count > 0:
                processed_files += 1
...
def main():
    parser = argparse.ArgumentParser(
        description="递归移除文件夹下所有文本文件中以 System.Diagnostics.Debug.WriteLine 开头的行"
    )
    parser.add_argument(
        'directory', 
        nargs='?', 
